# 🚑 Health & Metrics Endpoints (Laravel Template)
...
- `app_up` — app-level liveness
- `app_info` — app/env/version labels
- PHP memory usage (current + peak)
- `app_config_cached` — config cache status
- `app_routes_total` — number of routes
- `app_database_up` + `app_database_query_time_seconds`
- `app_redis_up` + `app_redis_ping_time_seconds`
- `app_metrics_scrapes_total` — per-worker scrape counter

---

## 🧩 Full Laravel Implementation (`routes/web.php`)
