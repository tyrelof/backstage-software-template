...
export function Status() {
  return (
    <div>
      ...
      <a href={process.env.NEXT_PUBLIC_API_URL}>API</a>
      
      {/* ❌ Cannot use DB_HOST here (server-only) */}
      {/* process.env.DB_HOST will be undefined */}
    </div>
  )
}
```
...
- [ ] Environment variables don't start with `NEXT_PUBLIC_` are server-only
